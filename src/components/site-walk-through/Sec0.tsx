...
const FEATURES: Feature[] = [
  { key: "camera", icon: "📹", title: "Live Camera Scan", desc: "Use the device camera (getUserMedia) to scan the site. Run in-browser YOLOv8 + MiDaS for object + depth. Optional bridge to ARCore (Capacitor) for accurate square footage.", note: "Requires camera permission" },
  { key: "room", icon: "🏠", title: "Auto Room Detection", desc: "Identify room type using YOLO or Grounding DINO + Segment Anything. Use prompts like “stove/sink/toilet” to auto-tag kitchen or bath." },
  { key: "debris", icon: "🧪", title: "Debris + Material Detection", desc: "Fine-tuned YOLO/classifier to detect drywall dust, tape, overspray, residue. Train on real site image examples." },
  { key: "measure", icon: "📏", title: "Auto Measure", desc: "ARCore + WebXR hit-testing to place anchors and measure. Fallback: detect door/tile, infer scale, estimate dimensions." },
];

const APP_HREF = "https://budoboost.8thwall.app/prod";

const s = {
  section: { maxWidth: 980, margin: "24px auto", padding: "16px" } as React.CSSProperties,
  header: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 12 } as React.CSSProperties,
  ...
  row: { display: "flex", alignItems: "flex-start", gap: 12, border: "1px solid #e6e6e6", borderRadius: 12, padding: 14, background: "#fff" } as React.CSSProperties,
  ...
  button: { display: "inline-flex", alignItems: "center", justifyContent: "center", padding: "10px 14px", borderRadius: 10, border: "1px solid #111", background: "#111", color: "#fff", cursor: "pointer", textDecoration: "none", fontSize: 14, fontWeight: 600, whiteSpace: "nowrap" } as React.CSSProperties,
  footer: { marginTop: 16, color: "#444", fontSize: 13 } as React.CSSProperties,
  code: { background: "#f5f5f5", borderRadius: 6, padding: "2px 6px", fontFamily: "ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace", fontSize: 12 } as React.CSSProperties,
};

const Sec0: React.FC = () => {
  // Keep the same name/signature, but open in a new tab safely with fallback.
  ...
  return (
    <>
      <section
        ...
        style={{
          backgroundImage: `url(https://storage.googleapis.com/budoapps-5aacf.firebasestorage.app/templates/pressurewashing-3365cccdb5/images/bg_2.jpg)`,
        }}
      ...
      <section style={s.section} aria-label="Smart Site Walkthrough features">
        <header style={s.header}>
          ...
          <a
            href={APP_HREF}
            onClick={(e) => { e.preventDefault(); open(APP_HREF); }}
            style={s.button}
          >
            Open App
          </a>
