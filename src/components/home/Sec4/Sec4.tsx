...
const Sec4: React.FC = () => {
  ...
  // Embla (keep Owl classes so theme CSS applies)
  ...
  loop: true,
  align: "start",
  dragFree: true,            // ← free momentum drag (not forced to nearest snap)
  containScroll: "keepSnaps",// ← keep first/last snaps inside the viewport
  slidesToScroll: 1,    // ← when using next/prev or dots, move a whole “page”
  duration: 16,              // ← slightly quicker easing for a lively glide
});
  ...
  const handleSelect = useCallback(() => {
    ...
    // remove Owl fade by toggling "active" on visible slides
                              ...
                              <p className="rate" style={{ display: "flex", gap: 6 }}>
                                {Array.from({ length: 5 }).map((_, k) => (
                                  <FontAwesomeIcon
                                    key={k}
                                    icon={faStar}
                                    style={{ color: "#F9B233" }} // match theme yellow
                                  />
                                ))}
                              </p>
                              <p>{s.body}</p>
